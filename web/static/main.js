...
let friendGameoverChallengeView = 'compose';
...
function wireFriendsActions() {
  ...
  document.querySelectorAll('[data-friend-challenge-view]').forEach((btn) => {
    if (btn.closest('#friend-challenge-panel')) return;
    btn.addEventListener('click', () => {
      friendChallengeView = btn.dataset.friendChallengeView;
      renderFriendChallengePanel();
    });
  });
...
function wireFriendChallengePanelActions() {
  if (!els.friendChallengePanel) return;
  ...
  els.friendChallengePanel.querySelector('[data-friend-challenge-send]')?.addEventListener('click', async (event) => {
  ...
  els.friendChallengePanel.querySelectorAll('[data-friend-challenge-view]').forEach((btn) => {
    btn.addEventListener('click', () => {
      friendChallengeView = btn.dataset.friendChallengeView;
      renderFriendChallengePanel();
    });
...
function renderFriendChallengePanel() {
  ...
  const incoming = friendsData?.incoming_challenges?.find(
  ...
  if (incoming && friendChallengeView === 'incoming') {
    ...
    wireFriendChallengePanelActions();
    return;
  }
  els.friendChallengePanel.innerHTML = `
    ${incoming ? `<button class="secondary friend-challenge-toggle" type="button" data-friend-challenge-view="incoming">Incoming: ${escapeHtml(friendChallengeLabel(incoming))}</button>` : ''}
...
function renderFriendGameoverChallenge() {
  ...
  const incoming = friendsData?.incoming_challenges?.find((row) => row.user_id === selection.friendUserId);
  if (incoming && friendGameoverChallengeView === 'incoming') {
    els.friendGameoverChallenge.hidden = false;
    els.friendGameoverChallenge.innerHTML = `
      <div class="friend-gameover-title">${escapeHtml(incoming.name || selection.friendName)} challenged you</div>
      ...
      <div class="friend-gameover-actions">
        ...
        <button class="secondary" type="button" data-gameover-challenge-view="compose">Challenge</button>
  ...
  els.friendGameoverChallenge.innerHTML = `
    ...
    ${incoming ? `<button class="secondary friend-challenge-toggle" type="button" data-gameover-challenge-view="incoming">Incoming: ${escapeHtml(friendChallengeLabel(incoming))}</button>` : ''}
...
function wireFriendGameoverChallengeActions() {
  if (!els.friendGameoverChallenge) return;
  ...
  els.friendGameoverChallenge.querySelectorAll('[data-gameover-challenge-view]').forEach((btn) => {
    btn.addEventListener('click', () => {
      friendGameoverChallengeView = btn.dataset.gameoverChallengeView;
      renderFriendGameoverChallenge();
    });
  });
...
function openFriendGameoverChallenge() {
  ...
  friendGameoverChallengeView = 'compose';
  renderFriendGameoverChallenge();
}
...
function renderIncomingFriendGameoverChallenge() {
  ...
  friendGameoverChallengeSelection = {
    ...
    friendName: incoming.name || game.opponent_name || 'Friend',
    sport: incoming.sport || CURRENT_SPORT,
    mode: incoming.mode || 'dr',
  };
  friendGameoverChallengeView = 'incoming';
  renderFriendGameoverChallenge();
}
...
function makePlayerCard(player, isSeed, options = {}) {
  ...
  if (teamColors.length) {
    ...
    teamColors.forEach((color, index) => playerCard.style.setProperty(`--team-color-${index + 1}`, color));
    playerCard.style.setProperty('--team-color-2', teamColors[1] || teamColors[0]);
    playerCard.style.setProperty('--team-color-3', teamColors[2] || teamColors[0]);
  }
...
function frBoardPlacement(sport, unit, slot, index) {
  ...
  const layouts = {
    baseball: {
      0: { x: 84.5, y: 83.5, zone: 'sideline', label: 'DH' },
      1: { x: 72, y: 62, zone: 'surface' },
      2: { x: 50, y: 60, zone: 'surface' },
      3: { x: 61.5, y: 41, zone: 'surface' },
      4: { x: 28, y: 62, zone: 'surface' },
      5: { x: 38.5, y: 41, zone: 'surface' },
      6: { x: 25.5, y: 23, zone: 'surface' },
      7: { x: 50, y: 15.5, zone: 'surface' },
      8: { x: 74.5, y: 23, zone: 'surface' },
      9: { x: 50, y: 87, zone: 'surface' },
      10: { x: 15.5, y: 79.5, zone: 'sideline', label: 'RP' },
      11: { x: 15.5, y: 89.5, zone: 'sideline', label: 'CP' },
    },
    basketball: {
      0: { x: 59, y: 17, zone: 'surface', label: 'PG' },
      2: { x: 32, y: 49, zone: 'surface', label: 'SG' },
      4: { x: 87, y: 49, zone: 'surface', label: 'SF' },
      6: { x: 39, y: 78, zone: 'surface', label: 'PF' },
      8: { x: 79, y: 78, zone: 'surface', label: 'C' },
      1: { x: 9, y: 12.5, zone: 'sideline', label: 'PG' },
      3: { x: 9, y: 25, zone: 'sideline', label: 'SG' },
      5: { x: 9, y: 37.5, zone: 'sideline', label: 'SF' },
      7: { x: 9, y: 50, zone: 'sideline', label: 'PF' },
      9: { x: 9, y: 62.5, zone: 'sideline', label: 'C' },
      10: { x: 9, y: 75, zone: 'sideline', label: 'BN' },
    ...
    football: unit === 'defense'
      ? {
          0: { x: 26, y: 17, zone: 'surface', label: 'EDGE' },
          1: { x: 42, y: 17, zone: 'surface', label: 'DT' },
          2: { x: 58, y: 17, zone: 'surface', label: 'DT' },
          3: { x: 74, y: 17, zone: 'surface', label: 'EDGE' },
          4: { x: 34, y: 38, zone: 'surface', label: 'OLB' },
          5: { x: 50, y: 39, zone: 'surface', label: 'MIKE' },
          6: { x: 66, y: 38, zone: 'surface', label: 'OLB' },
          7: { x: 13, y: 29, zone: 'surface', label: 'CB' },
          8: { x: 87, y: 29, zone: 'surface', label: 'CB' },
          9: { x: 26, y: 58, zone: 'surface', label: 'S' },
          10: { x: 74, y: 58, zone: 'surface', label: 'S' },
          11: { x: 50, y: 72, zone: 'surface', label: 'P' },
        }
      : {
          0: { x: 50, y: 31, zone: 'surface', label: 'QB' },
          1: { x: 50, y: 14, zone: 'surface', label: 'RB' },
          2: { x: 10, y: 36, zone: 'surface', label: 'WR' },
          3: { x: 28, y: 32, zone: 'surface', label: 'SLOT' },
          4: { x: 90, y: 36, zone: 'surface', label: 'WR' },
          5: { x: 72, y: 32, zone: 'surface', label: 'TE' },
          6: { x: 24, y: 50, zone: 'surface', label: 'OT' },
          7: { x: 37, y: 50, zone: 'surface', label: 'OG' },
          8: { x: 50, y: 49, zone: 'surface', label: 'C' },
          9: { x: 63, y: 50, zone: 'surface', label: 'OG' },
          10: { x: 76, y: 50, zone: 'surface', label: 'OT' },
          11: { x: 50, y: 72, zone: 'surface', label: 'K' },
        },
  };
