...
DEFAULT_DONATION_URL = "https://donate.stripe.com/dRm00d6Uf1KB8RR3Kdf3a00"
DONATION_URL = os.environ.get("DONATION_URL", "").strip() or DEFAULT_DONATION_URL
...
LOCAL_PLAYOFF_CONFIG = {
    "basketball": {
        "powerups": {
            "heat_check": {"label": "Heat Check", "description": "Name a Player with a 2,000+ point season. +5 seconds.", "kind": "stat", "bonus_seconds": 5, "stat": "peak_points", "threshold": 2000},
            "sixth_man": {"label": "Sixth Man", "description": "Name a Player with 7,000+ career assists. +5 seconds.", "kind": "stat", "bonus_seconds": 5, "stat": "career_assists", "threshold": 7000},
            "switch": {"label": "Switch", "description": "Name a Player who played the same position. +5 seconds.", "kind": "same_position", "bonus_seconds": 5},
            "mvp_badge": {"label": "MVP Badge", "description": "Name an MVP winner. +5 seconds.", "kind": "stat", "bonus_seconds": 5, "stat": "mvp_count", "threshold": 1},
            "all_star_callup": {"label": "Star Power", "description": "Name an All-Star. +5 seconds.", "kind": "stat", "bonus_seconds": 5, "stat": "all_star_count", "threshold": 1},
    ...
    "football": {
        "powerups": {
            "trick_play": {"label": "Trick Play", "description": "Name a Player with a 20+ rushing or receiving touchdown season. +5 seconds.", "kind": "stat", "bonus_seconds": 5, "stat": "peak_rush_receiving_touchdowns", "threshold": 20},
            "iron_man": {"label": "Iron Man", "description": "Name a Player with 100 career games played. +5 seconds.", "kind": "veteran", "bonus_seconds": 5, "career_games": 100},
            "package_change": {"label": "Package Change", "description": "Name a Player who played the same position. +5 seconds.", "kind": "same_position", "bonus_seconds": 5},
            "mvp_badge": {"label": "MVP Badge", "description": "Name an MVP winner. +5 seconds.", "kind": "stat", "bonus_seconds": 5, "stat": "mvp_count", "threshold": 1},
            "pro_bowl_callup": {"label": "Bowler", "description": "Name a Pro Bowler. +5 seconds.", "kind": "stat", "bonus_seconds": 5, "stat": "all_star_count", "threshold": 1},
            "timeout": {"label": "Timeout", "description": "+15 seconds.", "kind": "time", "bonus_seconds": 15},
    ...
    "hockey": {
        "powerups": {
            "breakaway": {"label": "Breakaway", "description": "Name a Player with a 400+ goal career. +5 seconds.", "kind": "stat", "bonus_seconds": 5, "stat": "career_goals", "threshold": 400},
            "veteran_presence": {"label": "Veteran Presence", "description": "Name a Player with 800+ career points. +5 seconds.", "kind": "stat", "bonus_seconds": 5, "stat": "career_points", "threshold": 800},
            "line_change": {"label": "Line Change", "description": "Name a Player who played the same position. +5 seconds.", "kind": "same_position", "bonus_seconds": 5},
            "hart_honor": {"label": "Hart Honor", "description": "Name a Hart Trophy winner. +5 seconds.", "kind": "stat", "bonus_seconds": 5, "stat": "mvp_count", "threshold": 1},
            "all_star_callup": {"label": "All-Star", "description": "Name an All-Star. +5 seconds.", "kind": "stat", "bonus_seconds": 5, "stat": "all_star_count", "threshold": 1},
            "timeout": {"label": "Timeout", "description": "+15 seconds.", "kind": "time", "bonus_seconds": 15},
...
PLAYOFF_POWERUPS = {
    "bubblegum": {
        "label": "Bubblegum",
        "description": "Name a Player with a 40+ home run season. +5 seconds.",
        "kind": "skill",
        "bonus_seconds": 5.0,
        "role": "batter",
    },
    "pine_tar": {
        "label": "Pine Tar",
        "description": "Name a Player with a 200+ strikeout season. +5 seconds.",
        "kind": "skill",
        "bonus_seconds": 5.0,
        "role": "pitcher",
    },
    "bat_donut": {
        "label": "Bat Donut",
        "description": "Name a Silver Slugger. +5 seconds.",
        "kind": "skill",
        "bonus_seconds": 5.0,
        "role": "any",
    },
    "sunglasses": {
        "label": "Sunglasses",
        "description": "Name an All-Star. +5 seconds.",
        "kind": "skill",
        "bonus_seconds": 5.0,
        "role": "any",
    },
    "backup_mitt": {
        "label": "Backup Mitt",
        "description": "Name a Gold-Glover. +5 seconds.",
        "kind": "skill",
        "bonus_seconds": 5.0,
        "role": "any",
    },
...
def _friends_payload(conn, guest_id: str) -> dict:
    ...
    matched_fields = "sport_id, mode" if challenge_columns_ready else "'baseball' AS sport_id, 'dr' AS mode"
    matched = conn.execute(
        f"""SELECT challenge_id::text, game_id::text, {matched_fields}
             FROM dr_friend_challenges
...
def _upsert_friend_match_request(conn, sender_id: str, recipient_id: str, sport: str,
    ...
    sender_name = _guest_label(conn, sender_id)
    recipient_name = _guest_label(conn, recipient_id)
    if pending:
        conn.execute(
...
@app.route("/api/friends/rematch", methods=["POST"])
def friends_rematch():
    ensure_runtime_schema()
    data = request.get_json(silent=True) or {}
    ...
    with db() as conn:
        ...
        if not guest_id or not _require_user(conn, guest_id):
            return jsonify({"error": "account login required"}), 403
        row = conn.execute(
            """SELECT sport_id, mode, state, finished
                 FROM sport_online_games WHERE game_id=%s""",
            (game_id,),
        ).fetchone()
        if not row:
            return jsonify({"error": "game not found"}), 404
        sport, mode, blob, finished = row
        if not finished or not blob.get("friend_challenge_id"):
            return jsonify({"error": "friend rematch unavailable"}), 400
        if guest_id not in {blob.get("p1_guest_id"), blob.get("p2_guest_id")}:
            return jsonify({"error": "unauthorized"}), 403
        other_id = blob["p2_guest_id"] if guest_id == blob["p1_guest_id"] else blob["p1_guest_id"]
        a, b = _friendship_pair(guest_id, other_id)
        if not conn.execute(
...
def friends_challenge_respond():
    ...
    with db() as conn:
        ...
        if preference == "rematch":
            ...
            if not source:
                return jsonify({"error": "rematch source unavailable"}), 409
            source_sport, source_mode, source_blob, source_finished = source
            source_players = {source_blob.get("p1_guest_id"), source_blob.get("p2_guest_id")}
...
def _local_po_traits(conn: sqlite3.Connection, sport: str, player_id: str) -> dict:
    row = conn.execute(
        """SELECT p.primary_pos, COALESCE(NULLIF(pt.career_games, 0), s.career_games),
                  ...
                  COALESCE(MAX(st.rushing_touchdowns + st.receiving_touchdowns), 0),
    ...
    if not row:
        return {"position": "", "career_games": 0, "team_count": 0, "franchise_count": 0, "season_count": 0,
                ...
                "peak_rush_receiving_touchdowns": 0,
    ...
    return dict(zip(("position", "career_games", "team_count", "franchise_count", "season_count",
                     ...
                     "peak_rush_receiving_touchdowns",
...
def _local_po_stat_label(stat_key: str | None) -> str:
    labels = {
        ...
        "peak_rush_receiving_touchdowns": "rushing or receiving touchdowns in one season",
...
def _local_po_powerup_requires_franchise(meta: dict) -> bool:
    return meta.get("requires_franchise", True) is not False
...
def _local_po_powerup_move(conn: sqlite3.Connection, game: dict, raw: str, player_id: str | None) -> dict | None:
    ...
    requires_franchise = _local_po_powerup_requires_franchise(meta)
    eligible = bool(current_franchises) if requires_franchise else True
    kind = meta.get("kind")
    if kind == "veteran":
    ...
    if not requires_franchise:
        state.chain.append(candidate_id)
        state.chain_names.append(name)
        state.chain_shared_with_prev.append([])
        ...
        return {"outcome": "valid", "player_id": candidate_id, "display_name": name, "disambiguation": disambiguation,
                "ambiguous_count": ambiguous_count, "shared_seasons": [], "burned_seasons": [],
...
def _sport_online_state(conn, game_id: str, blob: dict, state: GameState, viewer: str) -> dict:
    ...
    if blob["mode"] == "po":
        ...
        output["powerups"] = {"your_powerups": pp(side), "opponent_powerups": pp(other),
            "active_turn_powerup": ({"key": blob["active_turn_powerup"], "label": powers.get(blob["active_turn_powerup"], {}).get("label", blob["active_turn_powerup"])} if blob.get("active_turn_powerup") else None),
            "turn_powerup_used": bool(blob.get("turn_powerup_used")),
            "opening_lock_moves": PLAYOFF_OPENING_LOCK_MOVES}
    ...
    if sport != "baseball":
        ...
        if meta and meta.get("kind") not in {"time", "pressure"}:
            ...
            trait_order = {
                ...
                "peak_rush_receiving_touchdowns": "COALESCE(MAX(st.rushing_touchdowns + st.receiving_touchdowns), 0)",
