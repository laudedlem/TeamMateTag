"""Resolve remaining NBA headshot gaps from web image search.

This is a playtest-oriented fallback after NBA.com, Basketball-Reference,
ESPN, TheSportsDB, BBGM, and Wikimedia. It uses DuckDuckGo image results,
requires exact normalized name evidence in the title/image/source URL, validates
the image bytes, skips generic stock pages unless they expose a direct image,
and avoids ambiguous duplicate names unless a team cue is present.
"""
...
REPORT = ROOT / "raw" / "nba_web_image_headshots.csv"
USER_AGENT = "Mozilla/5.0 TeamMateTag NBA headshot resolver/0.2.10"
    ...
    with server.db() as conn:
        rows = conn.execute(
            """SELECT p.player_id,p.display_name,p.debut_year,p.final_year,p.primary_pos,
                ...
                WHERE p.sport_id='basketball' AND h.status IN ('placeholder','missing')
    ...
    return [
        {
            ...
            "position": pos or "", "teams": teams or [], "ambiguous_name": name_counts[name] > 1,
        }
...
def result_score(row: dict, item: dict) -> int:
    ...
    if "headshot" in text:
        score += 8
    if "portrait" in text or "poses" in text:
        score += 5
    if "nba.com" in text or "basketball-reference" in text or "realgm" in text or "gettyimages" in text:
        score += 4
...
def reject_reason(image: dict, hashes: set[str], perceptual: set[str]) -> str | None:
    ...
    if image.get("sha256") in hashes:
        return "known NBA placeholder hash"
    if any(hamming(image.get("perceptual_hash", ""), phash) <= 4 for phash in perceptual):
        return "known NBA placeholder perceptual match"
...
def candidate(session: requests.Session, row: dict, hashes: set[str], perceptual: set[str]) -> dict:
    query = f'"{row["name"]}" basketball headshot portrait'
    results = [item for item in image_results(session, query) if name_evidence(row, item)]
    results.sort(key=lambda item: result_score(row, item), reverse=True)
    notes = []
    for item in results[:8]:
        ...
        if reason:
            ...
            continue
        return {
            **row, **image, "status": "verified", "source_url": image_url,
            "source_page": item.get("url") or "", "title": item.get("title") or "",
            "note": f"Web image search exact-name match; score {score}; query {query}.",
        }
    return {**row, "status": "needs_review", "source_url": "", "source_page": "", "title": "", "note": " | ".join(notes[:5]) or "no exact-name image result"}
        ...
        with server.db() as conn, conn.cursor() as cur:
            cur.executemany(
                """INSERT INTO player_headshot_source_attempts (sport_id,player_id,provider,status,source_url)
                   VALUES ('basketball',%s,'Web image search','verified',%s)
