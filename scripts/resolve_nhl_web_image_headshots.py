"""Resolve remaining NHL headshot gaps from web image search.

This is the hockey equivalent of the NBA web-image fallback. It is intended
for playtesting coverage after NHL.com, Hockey-Reference, ESPN, FHM packs,
TheSportsDB, and Wikimedia have been exhausted.
"""
...
REPORT = ROOT / "raw" / "nhl_web_image_headshots.csv"
USER_AGENT = "Mozilla/5.0 TeamMateTag NHL headshot resolver/0.2.10"
    ...
    with server.db() as conn:
        rows = conn.execute(
            """SELECT p.player_id,p.display_name,p.debut_year,p.final_year,p.primary_pos,
                ...
                WHERE p.sport_id='hockey' AND h.status IN ('placeholder','missing')
...
def placeholder_fingerprints() -> tuple[set[str], set[str]]:
    ...
    for url in KNOWN_PLACEHOLDER_URLS["hockey"]:
        image = fetch(url)
...
def team_cue(row: dict, text: str) -> str | None:
    ...
    for team in row["teams"]:
        ...
        if len(nickname) > 3 and nickname in lower:
            return team
    return None
...
def result_score(row: dict, item: dict) -> int:
    ...
    if "portrait" in text or "poses" in text:
        score += 5
    if "hockeydb" in text or "eliteprospects" in text or "nhl.com" in text or "espn" in text:
        score += 6
    if "gettyimages" in text or "alamy" in text or "zimbio" in text:
        score += 4
...
def reject_reason(image: dict, hashes: set[str], perceptual: set[str]) -> str | None:
    ...
    if image.get("sha256") in hashes:
        return "known NHL placeholder hash"
    if any(hamming(image.get("perceptual_hash", ""), phash) <= 4 for phash in perceptual):
        return "known NHL placeholder perceptual match"
...
def candidate(session: requests.Session, row: dict, hashes: set[str], perceptual: set[str]) -> dict:
    queries = [
        f'"{row["name"]}" hockey headshot portrait',
        f'"{row["name"]}" NHL headshot',
    ]
    notes = []
    seen = set()
    candidates = []
...
def main() -> None:
        ...
        with server.db() as conn, conn.cursor() as cur:
            cur.executemany(
                """INSERT INTO player_headshot_source_attempts (sport_id,player_id,provider,status,source_url)
                   VALUES ('hockey',%s,'Web image search','verified',%s)
    ...
    print(f"Promoted {len(promoted)} NHL web-image headshots.")
