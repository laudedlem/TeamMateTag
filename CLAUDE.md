...
- `0.5.12`: refined cross-year player-card stint labels to use actual
  first/last game dates when compact stints have them. Basketball and Hockey
  compact rebuilds now carry date-backed player-team stints from local
  game-level appearance proof DBs, so a team stint ending before January 1
...
- `0.5.22`: Playoffs bots now strongly avoid planned timeout losses before
  powerups unlock or while holding unused powerups, making early no-powerup
...
- `0.5.23`: Playoffs bots no longer take planned timeout losses before
  ...
  flatter button.
- `0.5.24`: Desktop Playoffs expanded Powerups now reuses the same summary pill
  styling as the collapsed state so `Powerups`, unused count, and opponent count
  do not shrink or spread out when opened.
- `0.5.25`: Desktop Playoffs now widens the right rail while side-rail info is
...
- `0.5.29`: Browser/client timeout expiry no longer finishes bot turns; bot
  losses must go through the bot timeout path so Playoffs bots are not killed by
...
- `0.5.36`: Hotfixed non-baseball Playoffs powerup move handling. Invalid
...
- `0.5.38`: Corrected Hockey Playoffs `Breakaway` and `Veteran Presence` so
  they are pure stat-qualified powerup links, not teammate or same-franchise
...
- `0.5.42`: Forced Hockey Playoffs bots to attempt playable non-clock powerups
  after the opening lock until their set is spent, made powerup feedback more
  visible, added per-player win-condition contribution badges, and updated the
  legacy Playoffs queue dropdown copy to current targets.
- `0.5.43`: Tightened Hockey Playoffs bot powerup behavior so bots try playable
...
- `0.5.46`: Applied the same season-to-date upload guard to the shared
  ...
  start.
- `0.5.47`: Added an empty-season upload guard for MLB and the shared
  Basketball/Hockey live updater. If a season-to-date build finds zero
...
Update, 2026-08-28 (0.4.21): Manager hub sport tiles briefly moved the starter
headshot/name under the sport copy to avoid Basketball crowding. This layout
was superseded by `0.4.22`.
Update, 2026-08-28 (0.4.22): Reverted Manager hub sport tiles back to the
side-by-side layout the user preferred. The Today's Starter block now stays in
the open lane between the sport name and the bottom-right sport icon, with
mobile-specific sizing so Basketball title, starter headshot/name, and icon do
...
participation data from NFL Game Book PDFs. The script uses nflverse
`games.csv` for the 2000-2012 regular-season schedule, builds modern NFL.com
game URLs first, falls back to official NFLGSIS Gamebook paths by schedule
GSIS id when the modern page lacks a PDF, and supports explicit PDF overrides
for known edge cases. It validates every PDF against the scheduled teams before
parsing so wrong-but-valid fallback PDFs cannot be imported. The parser reads
first-page `Lineups` and `Substitutions`, handles inherited substitution
positions and hybrid labels such as `DT-LS`, excludes `Did Not Play`/`Not
Active`, and resolves abbreviated Game Book names to app Football IDs by
...
`2000_02_CLE_CIN`, `C.Clemons` in `2003_07_WAS_BUF`, and `V.Vaughn` in
...
explicit three-line stack: `Powerups`, `and Win`, `Conditions`.

Update, 2026-08-23 (0.3.96): enlarged the primary sport/mode names on the root
homepage launch tiles so they take up more of each button while preserving the
bottom-left status text and icon spacing.

Update, 2026-08-23 (0.3.95): improved root homepage launch tile readability.
Added black outline-style shadows to the white tile labels/status text, reduced
status copy size slightly, moved the icon area farther right, added more
right-side text padding, and constrained the Playoffs status so `Powerups and
Win Conditions` can wrap cleanly instead of overlapping its icon.

Update, 2026-08-23 (0.3.94): restored the root homepage launch tile status
copy, such as `Hit the Field`, to the bottom-left of each button after the
recent smoky tile styling accidentally centered/moved it.

Update, 2026-08-23 (0.3.93): adjusted the root homepage launch tiles after
playtesting the smoky background. The tiles now keep the smoky style but use
more light/dark variation from each tile's color, with thicker default colored
borders and an even bolder hover border.
...
a pill, and made the icons too heavy. The final cascade now uses smoky color
surfaces with a thicker border, restores the lighter icon line weight, and lets
the lower status text sit naturally again.
...
four-across desktop control deck with larger chips, bigger icons, and wrapping
labels/states so text does not get clipped. Opponent Powerups now sit as a
compact scouting strip below instead of reserving a large right-side column that
left dead horizontal space.
...
polish pass: Playoffs in-game win-condition and powerup panels are denser and
more mobile-friendly, Profile/Friends copy and cards follow the worksheet
capitalization/aesthetic more closely, Film Review archive rows include a date
column with clearer action/status grouping, and mode-hub archive dropdowns say
`Choose Tape...`.

Update, 2026-08-23 (0.3.84): completed a support-page aesthetic sweep after
the Film Review sport graphics were accepted for now. Contact, Privacy, Terms,
Reset Password, and the local Headshot Audit utility now use the current smoky
app background, colored/top-accent panels, tighter 8px card radii, clearer form
surfaces, and worksheet-style capitalization polish where the old utility copy
was still visibly plain.
...
inward from the sidelines, moved defensive backs inward so larger widgets do
not clip the sideline, widened safeties to stay between CB and OLB lanes, and
enlarged football widgets/headshots again while preserving two-line name room.

Update, 2026-08-23 (0.3.81): refined Football Film Review spacing again.
Offense keeps the RB high while moving QB, receivers, tight end, slot, and OL
up closer to that unused space; K moved up from the goalpost area and regained
a larger widget. Defense safeties now sit horizontally between the CB and OLB
lanes on each side, the defense stays higher overall, and P moved up from the
bottom post with a larger widget.

Update, 2026-08-23 (0.3.80): enlarged Football Film Review widgets and gave
the football board more vertical room. Offense now spreads vertically with the
RB higher in the unused upfield space, the QB separated from both RB and OL,
and the receiving group offset near QB depth. Defense shifted substantially
upfield, with the defensive line higher, linebackers and corners beneath it,
safeties clear of the uprights, and K/P kept in the end zone above the goalpost
bar.
...
25 yards plus a distinct bottom end zone, consistent sideline and inner hash
marks, a yellow goalpost in the end zone, and all 12 lineup players on the
field with no sideline lane. Offense is arranged as if attacking the bottom end
zone, Defense as if defending it, with the kicker/punter placed in the end
zone.
...
Update, 2026-08-23 (0.3.74): final hockey Film Review rink positioning tweak.
The offensive-zone faceoff circles/dots and `LW`/`RW` widgets moved slightly
closer to the side boards.

Update, 2026-08-23 (0.3.73): refined hockey Film Review details. The bench
lane is more distinct against the blue background, the awkward faceoff hash
marks were removed, and all four hockey faceoff dots are compensated as ellipses
so they render visually round in the stretched rink SVG.
...
moved slightly closer to the side boards, center moved a bit lower without
touching the crease, goalie moved higher between the goal line and end boards,
and bench widgets were enlarged again.

Update, 2026-08-23 (0.3.70): refined hockey Film Review styling. The outer
board background is now stronger hockey blue while the ice stays pale, the top
rink edge is drawn as a red center-line-style stroke, neutral-zone dots moved to
the midpoint between that top line and the blue line, and all hockey player
widgets were enlarged.

Update, 2026-08-23 (0.3.69): refined the hockey Film Review frame so the rink
no longer bleeds offscreen. The SVG now draws the cropped rink directly inside
the visible board with a consistent thin margin, keeps the neutral-zone dots
near the top, moves defensemen closer to the blue line, moves center slightly up
off the crease, places goalie between goal line and end boards, and enlarges
both bench and on-ice widgets.
...
than the board and shifts upward, trimming the empty top space while preserving
rink proportions. `LW`/`RW` moved closer to the side boards and the goalie moved
slightly lower.

Update, 2026-08-23 (0.3.66): compacted the hockey Film Review rink vertically.
The top crop is closer to the neutral-zone dots, the bench stack is tighter,
`LW`/`RW` moved up and outward above the offensive-zone faceoff dots, the center
moved up away from the crease, and the goalie moved up just below the goal line.

Update, 2026-08-23 (0.3.65): rebuilt the hockey Film Review board as an SVG
half-rink inspired by the user's reference. The new background has a full-height
left bench, rink boards immediately to the right, bottom icing/goal line, goalie
crease, two offensive-zone faceoff circles, a blue line, neutral-zone dots, and
retuned player placement for forwards, defensemen, goalie, and bench skaters.
...
`SG`/`SF` moved farther toward the wings, and `PF`/`C` moved slightly away from
the hoop.

Update, 2026-08-23 (0.3.62): nudged the basketball Film Review point-guard
starter widget down slightly so it sits between the top court line and the
three-point arc.

Update, 2026-08-23 (0.3.61): refined the basketball Film Review board after
the 0.3.60 background fix. The paint is narrower so it no longer looks
overstretched, the five starter widgets are spread out more across the court,
and the one-column bench stack is vertically compressed to fit inside the bench
lane.

Update, 2026-08-23 (0.3.60): fixed the basketball Film Review background
itself by disabling SVG aspect-ratio preservation. The court artwork now
stretches to the wide/short board instead of staying square-ish, with the bench
full-height on the left and the court occupying the remaining right side.

Update, 2026-08-23 (0.3.59): made the requested large basketball Film Review
layout adjustment. The bench now runs nearly full height on the left with small
top/bottom gaps, the court starts immediately to its right and stretches to the
right edge, the whole board is much shorter/wider, `PG` sits near the top court
line, and the three-point arc plus paint were reshaped for the smushed court.

Update, 2026-08-23 (0.3.58): corrected the basketball Film Review board after
the 0.3.57 bench/court pass pinched the court too far. The court now uses nearly
the full horizontal space again, the bench is a narrow one-column lane with a
small gap before the court, bench widgets are centered inside that lane, and the
overall board is shorter.
...
and the bench lane now runs from point-guard height to the baseline with smaller
centered bench widgets spaced inside it.

Update, 2026-08-23 (0.3.55): refined the basketball Film Review board after
playtesting. The board is shorter to remove excess top space, the three-point
arc is flatter, the rim/backboard order is corrected, and bench widgets are
smaller, centered, and evenly spaced inside the side bench lane.

Update, 2026-08-23 (0.3.54): rebuilt the basketball Film Review board as a
simple SVG half-court inspired by the user's reference, rotated so the
hoop/baseline sit at the bottom. The court now has a sideline/endline frame,
three-point arc, paint, free-throw circle, hoop/backboard, and a side bench
strip; starter and bench coordinates were retuned to sit on those landmarks.
...
Update, 2026-08-22 (0.3.28): the sport-specific Division Rivalry and Playoffs
queue screen now has its own visual treatment instead of the generic start
card. Player-card metadata and link labels are centered in the active game;
Film hub preview players are larger to use the available space.

Update, 2026-08-22 (0.3.29): fixed the sport queue class so its redesigned
layout is applied to the actual multiplayer setup rather than the Football
Film Review unit picker. The header brand now displays `TeamMateTag.Com` and
mode-specific How to Play dialogs no longer repeat their heading.

Update, 2026-08-22 (0.3.30): selecting any Film Review archive action closes
the archive. Non-Film player cards use team-stint columns around a centered
portrait and career summary; links and game-over panels have stronger color.
Update, 2026-08-22 (0.3.31): player cards now receive structured team stints
(team ID, span, and season count). The client uses the top three franchise
...
Update, 2026-08-22 (0.3.35): normalized player-card color and team-column
formatting across sports and game modes. Local and cached Film Review cards now
keep `team_stints`, so hockey and football show top-team background colors
...
Update, 2026-08-23 (0.3.38): applied the worksheet-style capitalization pass to
active game, queue, mode-hub, reference, and game-over copy. Playoffs powerup
buttons no longer say Ready during the opening countdown or while it is the
opponent's turn; they show contextual labels such as After Tipoff, Your Turn,
...
Update, 2026-08-23 (0.3.41): rebuilt the Film Review lineup board as actual
sport surfaces instead of a plain slot grid. Baseball now draws a diamond with
DH/bullpen space, basketball draws a half court with starters on the floor and
bench slots on the sideline, hockey draws a half rink with forward/defense/
goalie placement plus bench slots, and football draws offensive or defensive
half-field formations with kicker/punter on special-teams sideline. Boards now
open by default so correct guesses visibly fill the sport graphic as the lineup
is solved.

Update, 2026-08-23 (0.3.42): redid the Film Review board graphics around the
reference diagrams. The background surfaces are simple line-only diagrams with
home plate, hoop/baseline, hockey net/endboard, and football end zone at the
bottom. Background label text was removed from bench/dugout/bullpen/special
teams areas. Baseball Film Review now targets 12 players and 11 connections by
adding RP and CP bullpen slots from pitcher appearances; archive rebuilds should
regenerate baseball Film Review tapes for the new length. Football formations
...
Update, 2026-08-23 (0.3.44): rebuilt the baseball Film Review board as a
baseball-only cleanup pass after playtesting. The diamond now uses explicit
...
Update, 2026-08-23 (0.3.46): refined the SVG baseball Film Review diamond after
playtesting. Second base now sits inside the infield instead of near the
center-field wall, the board background uses the site's baseball red, the mound
is drawn as a mound/plate instead of another base, and simple no-text bullpen
and dugout graphics sit behind the RP/CP and DH areas.

Update, 2026-08-23 (0.3.47): tightened the baseball Film Review SVG layout.
The outfield wall is pushed higher to reduce dead top space while the
...
Update, 2026-08-23 (0.3.49): enlarged and spread the baseball Film Review board
chips. Baseball now gets slightly larger board spots and headshots than the
shared sport default, while the position coordinates are spread outward enough
to keep the larger chips clear on mobile and desktop.

Update, 2026-08-23 (0.3.50): made a final baseball Film Review board scale
pass. The baseball player chips and headshots are slightly larger again, the
position spread is widened to preserve mobile spacing, and the SVG field art is
scaled vertically so a little top/bottom empty space is cropped by the board.

Update, 2026-08-23 (0.3.51): changed the baseball Film Review board crop from
an SVG-only scale to an actual shorter board surface. The SVG now intentionally
